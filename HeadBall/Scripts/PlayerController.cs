...
public class PlayerController : MonoBehaviour {
	...
	/// <summary>
	/// Main player controller. It handles player movement, jump, collisions and also provides a few options to controll 
	/// the character by touch or keyboard keys.
	/// </summary>
	...
	[Range(20.0f, 40.0f)]
	public float moveSpeed = 30.0f;			//player movement speed
	...
	void Awake () {
		...
		startingPosition = new Vector3(startingX, -0.7f, 0);
	...
	void rotateShoe() {
		...
		if(GetComponent<Rigidbody>().linearVelocity.x >= rotateMinVelocity) {
			myShoe.transform.localEulerAngles = new Vector3(0, maxShoeTurn, 180);
		} else if(GetComponent<Rigidbody>().linearVelocity.x <= rotateMinVelocity * -1) {
	...
	void OnCollisionEnter (Collision other) {
		...
		Vector3 shootForce;
		bounceDir.Normalize();
		shootForce = Vector3.Scale(bounceDir, new Vector3(0.2f, 0.55f, 0.2f));
